...
<Post {...props.frontmatter} showSubtitle>
  ...
  <Paragraph>
    ...
    <Text code>useEffect</Text> hook, we got rid of the warning. 🎉 If you'd
    like to get the data when a user types something in the search box (e.g. to
    find a particular Pokemon), you can pass additional dependencies to the{' '}
    <Text code>[]</Text> dependency list on <Text code>useEffect</Text>.{' '}
  </Paragraph>
  <Paragraph breakParagraph>
    If you need to get the data on button click, create an async function
    without <Text code>useEffect</Text> hook and assign it to the{' '}
    <Text code>onClick</Text> event.
  </Paragraph>
  <Title level={4} transform="none">
